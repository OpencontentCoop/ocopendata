      ...
      <div class="modal-body">
        <div class="clearfix">
          <button type="button" class="close" data-dismiss="modal" data-bs-dismiss="modal" aria-hidden="true"
                  aria-label="{'Close'|i18n('bootstrapitalia')}" title="{'Close'|i18n('bootstrapitalia')}">&times;
          </button>
        </div>
...
<script id="tpl-query" type="text/x-jsrender">
  <div class="tab-pane" id="panel-{{:object_id}}">
    ...
    {/literal}{if $is_translation_enabled}{literal}
                <a href="#" class="btn btn-xs btn-outline-secondary me-3" data-trigger="translate">Traduci italiano in tutte le lingue</a>
              {/literal}{/if}{literal}
      ...
      let loadStaticQueries = function () {
        ...
        $.get('/opendata/check_queries?load=1&withField=' + withField + '&withError=' + withError, function (response) {
          ...
          $.each(response, function () {
            ...
            query.find('[data-trigger="translate"]').on('click', function (e) {
              let parent = $(this).parents('[data-query-group]')
              let itaField = parent.find('textarea[data-attribute_language="ita-IT"]')
              if (itaField.length > 0) {
                parent.find('textarea').each(function () {
                  let textarea = $(this)
                  if (textarea.data('attribute_language') !== 'ita-IT') {
                    $.ajax({
                      type: "GET",
                      url: '/opendata/check_queries',
                      data: {
                        translate: itaField.val(),
                        from: 'ita-IT',
                        to: textarea.data('attribute_language')
                      },
                      contentType: "application/json; charset=utf-8",
                      dataType: "json",
                      success: function (response) {
                      ...
                      error: function (data) {
                        var error = data.responseJSON;
                        alert(error?.error_message || 'error');
                      }
                    });
                  }
                })
