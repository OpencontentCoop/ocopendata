        ...
        <tr>
            ...
            {foreach $alias_list as $alias => $aliasData}
                <td{if $current_alias|eq($alias)} class="warning" {/if}>
                    <table>
                        <tr>
                            <td><a href="{'ckan/show/org?format=html'|ezurl(no)}" class="showButton btn btn-xs btn-{if is_set($aliasData['org'])}info{else}disabled{/if}">
                                    SHOW
                                </a></td>
                ...
                <tr>
                    <td>
                        <a href="{$node.url|ezurl(no)}">
                            {$node.name|wash()}
                            <br />
                            <small>{$node.contentobject_id} - {$node.remote_id|wash()} - {$node.object.remote_id|wash()}</small>
                        </a>
                    </td>
                    {foreach $alias_list as $alias => $aliasData}
                        <td{if $current_alias|eq($alias)} class="warning" {/if}>
                            <table>
                                ...
                                <tr>
                                    <td>
                                        <a href="{concat('ckan/show/',$node.contentobject_id,'?format=html')|ezurl(no)}" class="showButton btn btn-xs {if is_set($aliasData[$node.contentobject_id])}btn-info{else}btn-disabled{/if}">
                                            SHOW
                                        </a>
                                    </td>
                                    <td><a href="{concat('ckan/push/',$node.contentobject_id,'?format=html')|ezurl(no)}" data-action="push" data-alias="{$alias}" data-id="{$node.contentobject_id}" class="btn btn-xs btn-success">PUSH</a></td>
                                    <!--<td><a data-action="delete" data-alias="{$alias}" data-id="{$node.contentobject_id}" class="btn btn-xs btn-warning">DELETE</a></td>-->
                                    <td><a href="{concat('ckan/purge/',$node.contentobject_id,'?format=html')|ezurl(no)}" data-action="purge" data-alias="{$alias}" data-id="{$node.contentobject_id}" class="btn btn-xs btn-danger">PURGE</a></td>
                                </tr>
                            </table>
                        </td>
                    {/foreach}
...
//                    console.log(show);
                ...
                $('.showButton').click(function(e) {
                    var modal = $('#modal'), modalBody = $('#modal .modal-content');
                    modal
